...
printf "\n${BOLD}Claude Code — The Security Researcher${RESET}\n${CYAN}@seclab${RESET}\n\n"
...
for pkg in git gh node python@3.13 jq nmap openssl; do
...
You are working in a security-focused development environment.

# Priorities
- Flag any OWASP Top 10 issues immediately when spotted in code
- Never introduce SQL injection, XSS, SSRF, or command injection vectors
- Validate all external input at system boundaries
- Prefer allowlists over denylists for input validation

# Workflow
- Read relevant files before making changes
- For dependency changes: check CVE databases before adding new packages
- Document security assumptions and trust boundaries in code comments
- Ask before writing exploit code — ensure authorization context is clear
...
- Security-critical code deserves comments explaining the invariant being enforced
EOF
...
printf "\n${BOLD}${GREEN}The Security Researcher is ready.${RESET}\n\n"
printf "Next steps:\n"
